...
func checkCSRID(CSRID string) (*dao.CARequest, bool) {
	...
	csrIDStr, err := tools.AesDecrypt(csrIDBytes, key)
	if err != nil {
		return nil, false
	}
	csrID, err := strconv.Atoi(string(csrIDStr))
	if err != nil {
		return nil, false
	}
...
func AuditPassLogic(ctx *gin.Context, req ginTools.BaseReqInter) ginTools.BaseRespInter {
	...
	subjectKeyId := sha1.Sum([]byte(csr.PublicKey))
	rootCert.SubjectKeyId = subjectKeyId[:]
	...
	// 签发代码签名证书
	case definition.CertificateTypeCodeSign:
		ok = tools.CreateCodeSignCert(&rootCert, big.NewInt(int64(int(c.ID))), subject,
			csr.PublicKey, &rootPrvKey, notBefore, notAfter, crlDP, certFileName)
	// 签发 SSL 证书
	case definition.CertificateTypeSSL:
		dnsNames := strings.Split(csr.DnsNames, " ")
		ok = tools.CreateSSLCert(&rootCert, big.NewInt(int64(int(c.ID))), subject,
			csr.PublicKey, &rootPrvKey, notBefore, notAfter, crlDP, dnsNames, certFileName)
	default:
		resp.Header = ginTools.ParamErrorRespHeader
		return resp
	}
