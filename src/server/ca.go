...
func CaUploadPKLogic(ctx *gin.Context, req ginTools.BaseReqInter) ginTools.BaseRespInter {
	...
	// AES 解密
	key := []byte(src.GetSetting().Secret.ResponseSecret)
	csrIDString, err := tools.AesDecrypt(msgSplit, key)
	if err != nil {
		resp.Header = ginTools.ParamErrorRespHeader
		return resp
	}
	...
	// 转换成 int
	csrID, err := strconv.Atoi(string(csrIDString))
	if err != nil {
		resp.Header = ginTools.ParamErrorRespHeader
		return resp
	}
...
func CaCodeSignCsrLogic(ctx *gin.Context, req ginTools.BaseReqInter) ginTools.BaseRespInter {
	...
	// AES 加密返回的 ID
	encryptID, err := tools.AesEncrypt([]byte(strconv.Itoa(int(newCSR.ID))), []byte(src.GetSetting().Secret.ResponseSecret))
	if err != nil {
		resp.Header = ginTools.SystemErrorRespHeader
		return resp
	}
...
func CaCSRFileLogic(ctx *gin.Context, req ginTools.BaseReqInter) ginTools.BaseRespInter {
	...
	resp.EmailAddress = csr.Subject.Names[6].Value.(string)
	// 从 CSR 文件中获取公钥
...
func CaSslCsrLogic(ctx *gin.Context, req ginTools.BaseReqInter) ginTools.BaseRespInter {
	...
	//encryptID, err := tools.EncryptWithDES(strconv.Itoa(int(newCSR.UserID)), src.GetSetting().Secret.ResponseSecret)
	encryptID, err := tools.AesEncrypt([]byte(strconv.Itoa(int(newCSR.UserID))), []byte(src.GetSetting().Secret.ResponseSecret))
	if err != nil {
		resp.Header = ginTools.SystemErrorRespHeader
		return resp
	}
