...
func GetCRSByUID(id uint) (*CARequest, bool) {
	csr, err := selectCRSByUID(id)
	if err != nil || csr.UserID != id {
		msg := fmt.Sprintf("Fail to select crs by uid(%d)", id)
		tools.ExceptionLog(err, msg)
		return nil, false
	}
	return csr, true
...
func selectCRSByUID(id uint) (crs *CARequest, err error) {
	crs = &CARequest{}
	err = daoUtils.GetDB().Where("user_id = ?", id).First(crs).Error
	return crs, err
}
